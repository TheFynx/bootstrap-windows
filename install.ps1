...
# Modifications Copyright 2016 Levi Smith
#
...
$targetChefDk = '0.16.28'
$bootstrapWindows = 'bootstrap_windows'

function promptContinue {
  param ($msg="bootstrap_windows encountered an error")
...
function die {
  param ($msg="bootstrap_windows encountered an error. Exiting")
  Write-host "$msg."
  Break
}

if ($args[0]) {
  $bootstrapWindows = $args[0]
}
...
$tempInstallDir = Join-Path -path $env:TEMP -childpath 'bootstrap_windows'
...
$privateSource
cookbook '$bootstrapWindows', git: 'https://github.com/TheFynx/bootstrap-windows.git'
"@
...
2. Download the bootstrap_windows cookbook via Berkshelf
...
if ($env:BOOTSTRAP_WINDOWS_JSON_ATTRIBUTES) {
  chef-client -A -z -l error -c $chefConfigPath -o $bootstrapWindows --json-attributes $env:BOOTSTRAP_WINDOWS_JSON_ATTRIBUTES
}
else {
  chef-client -A -z -l error -c $chefConfigPath -o $bootstrapWindows
}
...
Write-Host "`n`nCongrats!!! Workstation now configured."
